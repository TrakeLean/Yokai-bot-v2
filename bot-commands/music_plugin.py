...
async def _join(ctx: lightbulb.Context) -> Optional[hikari.Snowflake]:
    ...
    connection_info = await music_plugin.bot.d.lavalink.wait_for_full_connection_info_insert(ctx.guild_id)
...
async def join_subcommand(ctx: lightbulb.Context) -> None:
...
@lightbulb.implements(lightbulb.SlashSubCommand, lightbulb.PrefixSubCommand)
async def leave_subcommand(ctx: lightbulb.Context) -> None:
...
@lightbulb.implements(lightbulb.SlashSubCommand, lightbulb.PrefixSubCommand)
async def play_subcommand(ctx: lightbulb.Context) -> None:
    ...
    con = music_plugin.bot.d.lavalink.get_guild_gateway_connection_info(ctx.guild_id)
    ...
    query_information = await music_plugin.bot.d.lavalink.auto_search_tracks(query)
    ...
    try:
        ...
        await music_plugin.bot.d.lavalink.play(ctx.guild_id, query_information.tracks[0]).requester(ctx.author.id).queue()
    except lavasnek_rs.NoSessionPresent:
        await ctx.respond(f"Use `{c.PREFIX}join` first")
        return
...
@lightbulb.implements(lightbulb.SlashSubCommand, lightbulb.PrefixSubCommand)
async def stop_subcommand(ctx: lightbulb.Context) -> None:
...
@lightbulb.implements(lightbulb.SlashSubCommand, lightbulb.PrefixSubCommand)
async def skip_subcommand(ctx: lightbulb.Context) -> None:
...
@lightbulb.implements(lightbulb.SlashSubCommand, lightbulb.PrefixSubCommand)
async def pause_subcommand(ctx: lightbulb.Context) -> None:
...
@lightbulb.implements(lightbulb.SlashSubCommand, lightbulb.PrefixSubCommand)
async def resume_subcommand(ctx: lightbulb.Context) -> None:
...
@lightbulb.implements(lightbulb.SlashSubCommand, lightbulb.PrefixSubCommand)
async def now_playing_subcommand(ctx: lightbulb.Context) -> None:
...
async def voice_server_update(event: hikari.VoiceServerUpdateEvent) -> None:
    await music_plugin.bot.d.lavalink.raw_handle_event_voice_server_update(event.guild_id, event.endpoint, event.token)
